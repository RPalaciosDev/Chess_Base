...
ChessEval::ChessEval() : castleStatus(0),
                         currentTurnNo(0),
                      ...
                      const PositionContext &context)
{
    ...
    if (std::abs(stockfish_eval) > 5000)
    { // Or use a constant like MAX_EVAL
...
bool ChessEval::loadModel(const std::string &filename)
{
    ...
    std::cout << "  Error Reduction:     " << (100.0f * (1.0f - metrics.running_average_error / metrics.initial_average_error)) << "%" << std::endl;
    std::cout << "=====================\n"
              << std::endl;
